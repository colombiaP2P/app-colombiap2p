...
function getXPLevel(xp) {
    for (let i = C2P_XP_LEVELS.length - 1; i >= 0; i--) {
        if (xp >= C2P_XP_LEVELS[i].min) return C2P_XP_LEVELS[i];
    }
...
async function loadUserProfile() {
    ...
    // [C2P FASE 3] Cargar sellos y badges desde PocketBase (no bloquea)
    loadUserStamps();
    loadUserBadges();

    // [C2P FASE 7] Rachas + Referidos
    if (typeof C2P_Rachas !== 'undefined') C2P_Rachas.init().catch(() => {});
...
function updateProfileDisplay() {
    ...
    if (isNpubFormat(pubKey)) {
        ...
        document.getElementById('profileId').textContent = `C2P-${shortHex}`;
        ...
        document.getElementById('profileId').textContent = `C2P-${shortId}`;
...
// ============================================
// [C2P FASE 3] Sellos y Badges desde PocketBase
// ============================================
const _RARITY_LABEL = { common: 'Común', rare: 'Raro', epic: 'Épico', legendary: 'Legendario' };

function _renderStampCard(stamp, obtained_at) {
...
// [C2P FASE 4] Badge rendering
const _SKILL_META = {
    lightning:  { icon: '⚡', color: '#F7931A', label: 'Lightning' },
    nostr:      { icon: '🟣', color: '#9B59B6', label: 'Nostr'     },
    bitcoin:    { icon: '₿',  color: '#F7931A', label: 'Bitcoin'   },
    privacidad: { icon: '🔒', color: '#2ECC71', label: 'Privacidad'},
    nodos:      { icon: '🖥️', color: '#3498DB', label: 'Nodos'     },
    p2p:        { icon: '🤝', color: '#E74C3C', label: 'P2P'       },
    educacion:  { icon: '📚', color: '#F39C12', label: 'Educación' },
    comunidad:  { icon: '🏘️', color: '#1ABC9C', label: 'Comunidad' },
};
const _LEVEL_DOTS = { basico: '●○○', intermedio: '●●○', avanzado: '●●●' };
const _LEVEL_LABEL = { basico: 'Básico', intermedio: 'Intermedio', avanzado: 'Avanzado' };

function _renderBadgeCard(badge, obtained_at) {
    const skill = _SKILL_META[badge.skill_area] || { icon: '🎖️', color: '#8a8178', label: badge.skill_area || '' };
    const level = badge.level || 'basico';
    const dots  = _LEVEL_DOTS[level] || '●○○';
    const date  = obtained_at
        ? new Date(obtained_at).toLocaleDateString('es-CO', { month: 'short', year: 'numeric' })
        : '';
...
async function loadUserStamps() {
    const listEl   = document.getElementById('c2pSellosList');
    const emptyEl  = document.getElementById('c2pSellosEmpty');
    ...
    listEl.innerHTML = stamps.map(({ stamp, obtained_at }) => _renderStampCard(stamp, obtained_at)).join('');
    listEl.style.display = '';
    if (emptyEl) emptyEl.style.display = 'none';
}

async function loadUserBadges() {
    const listEl  = document.getElementById('c2pBadgesList');
    const emptyEl = document.getElementById('c2pBadgesEmpty');
    ...
    listEl.innerHTML = badges.map(({ badge, obtained_at }) => _renderBadgeCard(badge, obtained_at)).join('');
    listEl.style.display = '';
    if (emptyEl) emptyEl.style.display = 'none';
}
